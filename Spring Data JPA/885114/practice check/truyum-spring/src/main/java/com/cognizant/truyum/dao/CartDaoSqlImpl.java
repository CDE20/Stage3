...
public class CartDaoSqlImpl implements CartDao {
	...
	public Set<MenuItem> getAllCartItems(long userId) throws CartEmptyException {
		...
		String query = "select * from cart c " + "inner join menu_item m " + "on c.cart_id = m.menu_item_id "
				+ "where c.user_id = ?";
		try {
			PreparedStatement preparedStatement = connection.prepareStatement(query);
			preparedStatement.setLong(1, userId);
			...
			query = "select sum(m.price) as total " + "from cart c " + "inner join menu_item m "
					+ "where c.user_id = ?";
